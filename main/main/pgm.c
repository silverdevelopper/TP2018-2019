...
PGMInfo pgm_read(const char *filename) {
    ...
    switch (pgm_info.signature[1]) {
	    ...
	    // Ipucu: for dongusu icinde fgets !!
            for (i=0; fgets(line, pgm_info.height*pgm_info.width, pgm)!=NULL; i++) {
	  ...
	  // Ipucu : for dongusu olmadan tek satirda fread !!
            fread(pgm_info.pixels, strlen(pgm_info.pixels), 1, pgm);
          break;
    }
...
int pgm_write(const char *filename, PGMInfo pgm_info) {
    ...
    // Alistirma 2.b
      fprintf(pgm, "%s\n%s\n%d %d\n",pgm_info.signature,pgm_info.comment,pgm_info.width,pgm_info.height);
    ...
    switch (pgm_info.signature[1]) {
        ...
        case '5':
            ...
            fwrite(pgm_info.pixels, 1, sizeof(pgm_info.pixels), pgm);
          break;
